...
} = require("./src/functions/getDataArduinos");
...
app.use(
  "/images-vehicles",
  express.static(path.join(__dirname, "public/upload/vehicles"))
);
...
app.use(
  "/files-lgpd",
  express.static(path.join(__dirname, "public/sharedFiles/lgpd"))
);
app.use(
  "/files-manuais-informatica",
  express.static(path.join(__dirname, "public/sharedFiles/manuaisinformatica"))
);
app.use(
  "/files-arquivos-downloads",
  express.static(path.join(__dirname, "public/sharedFiles/arquivosdownloads"))
);
app.use(
  "/files-its",
  express.static(path.join(__dirname, "public/sharedFiles/sgi/its"))
);
app.use(
  "/files-comunicacoes",
  express.static(path.join(__dirname, "public/sharedFiles/comunicacoes"))
);
app.use(
  "/files-noticias",
  express.static(
    path.join(__dirname, "public/sharedFiles/comunicacoes/noticias")
  )
);
...
//cria a conxão com o servidor do arduíno
getSensorDataRackSalaTI();
//faz a requisição dos dados dos sensores a cada 60 segundos
monitorSensorDataRackSalaTI(60);
//Verifica os dados recebidos do sensor a cada 10 segundos para disparo de notificações de alerta por email
rackSalaTI(60 * 10);
